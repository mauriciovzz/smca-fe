...
export const spaceInstanceLoader = async (auth, params, loaderErrors) => {
  if (!auth) {
    return redirect('/');
  }
...
const SpaceInstanceRoot = () => {
  ...
  const updateSpaceInstanceRoot = () => revalidator.revalidate();
...

        {
          !isMapRoute && (
            <div className={`${isScreenSmall ? 'order-1' : 'order-2'} flex grow`}>
              <Outlet context={{ spaceData, updateSpaceInstanceRoot, errorHandler }} />
            </div>
          )
        }
      </div>

      {isMapRoute && <Outlet context={{ spaceData, updateSpaceInstanceRoot, errorHandler }} />}
    </>
  );
};

export default SpaceInstanceRoot;
