...
const VariableItem = ({ nodeLocation, variable }) => {
  ...
  const valueColor = (value) => (value ? "bg-lime-400" : "bg-red-600");
      ...
      .then((requestedReadings) => {
        ...
        for (let i = 0; i < 24; i += 1) {
          const readingMatch = requestedReadings.find(
            (reading) => reading.end_hour === `${i}:00:00`,
          );
        ...
        <div className="flex w-full divide-x divide-bluey">
          {averageReadings.map((reading) => (
            <div
              ...
              data-tooltip-content={`${reading.startTime} - ${
                reading.endTime
              } : ${reading.value === null ? "no reading" : reading.value}`}
